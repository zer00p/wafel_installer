...
CFWVersion testCFW() {
    ...
    if (IS_CEMU_PRESENT()) {
        WHBLogPrint("Detected that Cemu is being used to run Wafel Installer...");
    ...
    if (ret == MOCHA_RESULT_SUCCESS) {
        if (mochaVersion == (1 + 1337)) {
            ...
            WHBLogPrintf("Running in Wafel Installer environment, all devices allowed");
        ...
        if (forceTiramisu == 1) {
            if (stopMochaServer()) {
                ...
                WHBLogPrint("");
                WHBLogPrint("Exiting Wafel Installer in 10 seconds...");
                WHBLogFreetypeDraw();
                sleep_for(10s);
            ...
            WHBLogFreetypeClear();
            WHBLogPrint("Exiting Wafel Installer...");
    ...
    WHBLogPrint("Attempt to use internal Wafel Installer CFW...");
    WHBLogFreetypeDraw();
    currCFWVersion = CFWVersion::NONE;
    return currCFWVersion;
}
